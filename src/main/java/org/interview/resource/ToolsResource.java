...
public class ToolsResource {
    ...
    @APIResponses(value = {
        ...
        @APIResponse(responseCode = "404", description = "Not Found", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorMessageDTO.class)))
    })
    public Response lookupDomain(
        @Parameter(description = "Domain name", required = true) @QueryParam("domain") String domain) {
        try {
            LookupResponseDTO response = domainLookupService.lookup(domain);
            return Response.ok(response).build();
        } catch (DomainNotFoundException e) {
            return Response.status(Response.Status.NOT_FOUND)
                .entity(new ErrorMessageDTO("Domain not found")).build();
        } catch (InvalidDomainException e) {
            return Response.status(Response.Status.BAD_REQUEST)
                .entity(new ErrorMessageDTO("Invalid domain format")).build();
        } catch (Exception e) {
            Log.error("Internal server error", e);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(new ErrorMessageDTO("Internal server error")).build();
        }
    }
}
